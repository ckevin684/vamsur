...
- **단일 파일**: `index.html` (~5680줄, ~289KB) — 모든 코드가 여기 한 곳에
...
1. `index.html` 수정
2. 검증 (프리뷰 하네스 등)
3. **🛑 커밋 전 반드시 프리뷰 스크린샷으로 사용자에게 먼저 보여주고 컨펌 받기** (비주얼/게임 변경 시)
4. 사용자 컨펌 후에만 **Claude가 `git commit` 실행** (의미있는 한국어 메시지)
5. 사용자가 GitHub Desktop에서 **Push만** 직접

> ✅ **Claude는 `git commit`까지 한다.**
...
| `zw` | 지워니 | 💵 동전던지기 | 골드 +50% | 📈 **주식 매매** — 처치로 시드머니 적립 → 100마다 자동매매 → 포트폴리오(데미지 배율) 확률 변동 |
...
- **📈 지워니(zw) 신규 캐릭터** (7번째): 동전던지기(coin)→비트코인(bitcoin, leverage 진화). `BAL.char.zw`. 시드머니(`p.seedMoney`, 처치 적립) 100마다 `doTrade()` → `p.portfolio`(데미지 배율) 확률변동(상승55/하락30/떡상10/폭락5). 하한0.5/상한2.5 가드. 포트폴리오는 fireWpn dmg에 곱. 패시브 골드×1.5. HUD `drawStockHUD()`(포트/시드/미니차트/매매팝업). 선택화면 `CCOLS=4`(7캐릭→4열2행). ⚠️ 비주얼(동전/HUD/선택카드)은 스샷 못떠서 미확인 — 플레이 검증 필요. EV+라 상한2.5 정착 경향(튜닝 여지).
...
## ⚖️ 밸런스 패치 세션 (하네스 측정 기반 — 최신)

> eval로 `update(dt)` 스텝 + 불사더미 스웜/단일 셋업으로 캐릭별 군중/단일 DPS 정량 측정. **하네스 핵심**: `spawnEnemies`·`askPlayerName` noop, `META.up` 0, `godMode`, dt=1/60×900스텝, 더미 `hp=1e9`·`spd=0`·매프레임 위치 재고정(`e.x=e.hx`). jh(바운싱램)는 노이즈 커서 3회 평균. 스웜=반경 30/55/80 더미 34개, 단일=정면 1개.

- **측정 결과(시작무기 Lv6 군중 스웜)**: gj 2170 ≫ jw 1530 > gh 1242 > co ~1100 > zw 940 > **jh 181💀** > **sm ~456(변신 룰렛 가중)**. jh가 압도적 최약체.
- **🔑 죽은 config 버그 2건 발견·수정 (BAL golden rule 위반)**:
  - `ram`: 실제 데미지가 하드코딩 `dmg*1.4`였고 `BAL.wpn.ram.dmgMul`은 **미사용**(죽은 값)이었음 → `dmg*BAL.wpn.ram.dmgMul`로 배선. fireWpn의 `dmg`엔 `dM`(레벨배율 `WLV.dmgM`)만 곱해질 뿐 무기별 dmgMul은 안 들어감 주의.
  - `applyStreamerLight`의 shibuki/riko/lize/tabi 빔 dmgMul도 **하드코딩**(huya만 `BAL.beam.huya.dmgMul` 참조였음) → 전부 `BAL.beam.X.dmgMul` 배선.
- **jh 대폭 버프**: `BAL.wpn.ram` `dmgMul 1.4→3.0`·`count 2→6`·`hitCd 0.30→0.22`. 군중 181→**840**. **🔑 바운싱램은 개수↑가 진짜 레버**(3개론 군중 커버리지 포화 → dmgMul 올려도 lv6 정체. 개수 늘려야 처리량 캡 돌파). jh는 최탱(ehp 144)+자가수리라 DPS 중하위가 정체성.
- **sm 일반폼 버프**: `BAL.beam` shibuki 1.4→2.0·riko 0.9→1.3·huya 2.0→2.6·lize 1.4→2.0. 일반폼 +30~45%, 변신가중 ~400→~456. ⚠️ 빔은 방향성(콘)이라 360° 스웜에선 과소평가 — 여전히 최하위라 실플레이 후 추가 여지.
- **jw 후반 너프**: `BAL.char.jw.lateScaleRate 0.3→0.2`. 15분 군중 4801→**3704**(내장 시간배율 3.25→2.5x). 무기레벨·진화·요리·콤보가 별도 곱이라 후반은 여전히 강함.
- **gj 유지**: 군중 2170 최강이나 최저이속(70)·최악 단일타겟(보스 67)·분노사이클 필요 트레이드오프로 미조정.
...
## 🆕 세션: 지워니 주식 대개편 + 뭉치 진화 리메이크 + 리롤 (최신)

> ⚠️ **이번 세션 내내 스크린샷 캡처가 인프라 문제로 막혀 있었음.** 로직은 전부 eval로 검증했지만 **모든 비주얼(투자창/상점/결과애니/황금뭉치/할퀴기/와이파이→발톱)은 미확인** — 다음 세션에도 막혀있으면 사용자 플레이 피드백으로 다듬어야 함.

### 📈 지워니(zw) = 주식 트레이더 (대개편 — 배율→현금→상점 모델)
- **현금 모델**: 처치→`p.seedMoney`(현금). 구 "포트폴리오 데미지 배율"은 **폐기**(데미지 직접배율 없음).
- **투자창** (`investOpen`, 룰렛식 일시정지): 주기 `BAL.char.zw.investInterval`(42s). 4종목(`STOCKS`: SAMSSUNG/APPRE/TESLAK/KAKOU, vol/drift 다름) 중 **전액 올인** 또는 **패스(현금 보유)**. `p.stocks`(가격 랜덤워크), `p.invest`{idx,amount,buyPrice,lev}. `doInvest`/`resolveInvest`/`stepStocks`.
- **정산 결과창** (`investResultOpen`, performance.now 애니): 움찔움찔(긴장)→스냅(매수가→현재가 ease-out-back, 흰플래시)→큰 손익 숫자 카운트업 + 떡상/떡락 라벨. `lastInvestResult`.
- **랜덤 상점** (`shopOpen`/`shopEnt`): 맵 스폰 `shopInterval`(185s, 15분 ~4-5회), 미니맵/월드 마커, 걸어가 진입. `SHOP_STATS`(공격/크리/공속/이동/범위/HP/관통) 4개 랜덤 + `SHOP_LEGENDARY`(추가거래소=아이템칸+1 `p.extraSlots`, 레버리지증서=`p.leverageNext`). **가격=현재 현금 비중**(스탯 15~23%, 전설 60~92%, 저점 100/400) → 부자면 스탯4개 전부 구매가능.
- **지원금**(`subsidy` 픽업): 현금 0 되면 맵 스폰(파산방지, `subsidyCash`60).
- **레버리지**: `p.leverageNext` → 다음 투자 변동 ×2(2배 먹고 2배 잃음).
- **무기 동전**: 던지기 아님 — **캐릭터 주위 공전(오비탈)**. 영구유지, 개수 동전2~6/비트코인7~11(진화가 항상 많게), 거리 `(65+lv*2.5)*(1+(am-1)*0.3)`(am 30%만 반영=안 멀어짐), 크기는 am 그대로. 회전 동전 비주얼(가로 cos 스쿼시).
- **진화 비트코인**: 동전 강화(개수·크기·데미지·속도↑) + **20초마다 사방 12방향 관통레이저**(`fireBitcoinLaser`, btclaser).
- **패시브 골드×1.5**. **분산투자**(구 leverage 패시브, 비트코인 진화재료) = 받는피해 **-7%/lv**(`p.dmgTakenMul`, 0.93). HUD `drawStockHUD`(현금/투자현황/타이머). 전부 `BAL.char.zw`.

### 🐕 뭉치 진화(언리쉬드) 리메이크
- 진화 시 **컴패니언 제거 → 플레이어가 황금 뭉치로 변신**. `drawCocker`(원래 흰 포메) + 은은한 금빛 틴트(가산혼합, 알파 낮음). (구 황금 UFO개·머리위 풍선/불꽃 제거됨)
- 공격: 짖기/할퀴기 폐기 → **앞으로 큰 할퀴기**(`wt:'wave'`): 70°(`BAL.wpn.unleash.waveSpan` 1.22rad) 콘 데미지(관통, waveDmgMul 2.2) + **평행 곡선 발톱자국 4줄** 비주얼(레이저 아님). 가까운 적 방향 자동.

### 🔄 레벨업 새로고침(리롤)
- 게임당 기본 2 + 메타 상점 구매(`META_UPGRADES.reroll`, max +8 → 총 10). `p.rerolls`(`applyMetaUpgrades`서 세팅).
- `triggerLvUp`→`rollLvUpCards`(분리) / `rerollLvUp`(횟수소모+재추첨). 레벨업화면 `rerollBtnRect` 버튼.

### 기타 수정
- **탱킹 너프**: 분산투자 -12%→-7%/lv, 기본재생 1.2→1.0, 진통제 1.5→1.2/lv (친구 첫 클리어가 "다 맞으며 깸" 피드백).
- **co 무기카드 버그**: mungchi 전용 제한 제거 → co도 다른 무기 주움.
- **뭉치 짖기 반경** `*player.am` 반영(범위 패시브).
- **WFAM 라벨**: coin/bitcoin/btclaser/wave 추가(전엔 동전이 '기타'로 뜸).
- **히나 폼명** 칸자키→**시라유키 히나**(스텔라이브 실명).
- 선택화면 `CCOLS=4`(7캐릭 2행).

### 🔑 다음 세션 우선 검토 (미완/논의중)
1. **진화 접근성** — 친구가 **진화 0개로 15분 클리어**. 무기4+아이템6=10개에 레벨업 분산→아이템 만렙 못함→진화 사실상 불가. 리롤로 일부 완화했지만 부족. **후보: 진화조건 완화(무기만렙+아이템Lv3) / 아이템 max 5→4 / 거의찬거 우선제시.** 아직 미적용.
2. **뭉치 "먹는 컷신"**(정지+변신) 미구현 — 황금뭉치 외형 확정 후 추가 예정.
3. **비주얼 전반** 사용자 플레이 검증 필요(스샷 불가였음): 황금뭉치·할퀴기·투자창/상점/결과애니·동전/레이저 생김새.
