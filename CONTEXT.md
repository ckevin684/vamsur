...
**🖥️ 램 무기 — 화면 튕기기 (뱀서 스타일)**: 던지는 게 아니라 가시 화면 영역 안에서 튕기는 RAM 스틱. 헬퍼 `spawnBouncingRam(p, opts)`. life 5s, cd 0.9s, 적당 0.5s 쿨다운 다회 타격. 발사당 개수: `1 + floor(wlv/3)` (Lv1=1, Lv8=3). 풀스펙은 `2 + floor(wlv/3)` + 빠른 속도 + 글로우.
...
**확률 분포 / 변신별 빔 차별화:**
- 🦊 텐코 시부키 (13%) — 주황 빔, 중간 폭(0.42rad), 15% 확률 폭딜 추가타
- 🗡️ 유즈하 리코 (13%) — 에메랄드 좁고 빠른 빔(halfAng 0.24)
- 🐲 사키하네 후야 (13%) — 보라 길고 좁은 레이저, 강한 단발
- 🦇 아카네 리제 (13%) — 빨간 빔, 슬로우 2.0초 길게, 처치 시 60% 확률 HP+3 흡혈
- 🎤 아라하시 타비 (13%) — 주황 빔 + 호밍 음표 동시 발사
- 💤 졸린 윤소민 (25% 꽝) — 회색 짧고 약한 빔, 30% 확률로 "불 안 켜짐"
- 👑 칸자키 히나 (10% **잭팟**) — 8방향 360° 거대 핑크/금 빔, 슬로우 3초, 처치 시 HP+1, 큰 화면 흔들림
