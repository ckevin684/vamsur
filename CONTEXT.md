...
- Player에 `bouncingRams=[]` 배열로 관리. fireWpn case 'ram'에서 wantCount 체크 + 능력치 동기화.
- 직사각형 모양 그대로 (회전 없음), 화면 가장자리에서 튕김.
- 뒤에 흰 트레일 잔상 (alpha 0.18, 8 포인트 × 0.04s 간격) — 가시성 안 망치는 정도.
- 레벨업 스케일: 데미지 (dmg×1.4), 속도 (220+wlv*10), 크기 (9+wlv*0.3) 모두 wlv에 비례.
- 진화 fullspec: 2개 + 더 빠르고 크고 발광 (#00eeff). 더 강한 데미지 (dmg×1.6).
- 적당 0.5초 재타격 쿨다운 (hitMap per enemy).
...
- **skeleton** (200초~) - 해골, **정지 포탑형**: 사정거리 도달 시 그 자리 고정 후 파란 마름모 투사체 사격 (`BAL.enemy.skel`)
- **splitter** (200초~) - 🆕 분열형 라임 블롭, 죽으면 작은 splitling 2마리로 쪼개짐
- **bomber** (300초~) - 🆕 자폭형 주황 폭탄(도화선), 죽으면 반경 72 광역 폭발(플레이어 피해) — 근접 처치 위험
...
- 모든 보스: 페이즈2 (HP 50% 이하 시, +히트스톱·흔들림), 텔레그래프 후 공격
- 처치 시: 회복 픽업 + 30% 폭탄 + 25% 자석 픽업 드랍 + 히트스톱

---

## 🪙 메타 진행 (영구 업그레이드 — VS 스타일)

- **골드**: 적/보스 처치로 적립(`runGold`, 일반1·탱크3·엘리트5·보스45) → HUD 카운터 → 판 종료 시 `bankRun()`이 `META.gold`에 적립, **localStorage `vamsur_meta`** 영속 저장.
- **강화 상점** (`state='shop'`, 타이틀 메뉴): 6종 영구 업그레이드(체력/공격/이동/경험치/크리/방어, 각 5레벨 비용곡선) + **🎰 룰렛 해금**(300골드).
  - 핵심 상수/함수: `META`, `META_UPGRADES`, `ROULETTE_COST`, `loadMeta/saveMeta/bankRun`, `applyMetaUpgrades(p)`(startGame에서 적용), `renderShop/getShopClick/shopBuy`.
- **룰렛 해금**: `META.roulette` → `rouletteOn(p)`가 소민 외 모든 캐릭터도 변신 룰렛 사용 가능. 비-소민은 `streamerBuff(p)`로 폼별 전무기 데미지 배율(잭팟 1.35·후야 1.3·일반 1.15·꽝 0.95). 소민은 기존 빔 변신 정체성 유지.

## ✨ 비주얼/가시성 (이번 세션)
- **히트스톱** `hitStop`: 큰 타격 순간 짧은 정지. 데미지 숫자 펀치-인 팝 + 크리 금색 글로우.
- **식별**: 플레이어 발밑 캐릭색 링, 보스 상시 아우라(금/페이즈2 빨강).
- **디버그 무적** `godMode`: 백틱(`) 키 또는 콘솔 `godMode=true`.

## 🧪 개발용 플레이테스트 하네스
- `serve.ps1`(PS 정적서버) + `~/.claude/launch.json`(8777) → Claude Preview로 직접 구동/스크린샷/eval. `update(dt)`를 eval 루프로 스텝해 실시간 대기 없이 DPS/TTK 정량 측정. (dev 전용, 배포 무관)
...
**확률 분포 / 변신별 빔 차별화 (밸런스 패치 완료):**
- 🦊 텐코 시부키 (~15.8%) — 주황 빔, 중간 폭(0.42rad), 15% 확률 폭딜 추가타
- 🗡️ 유즈하 리코 (~15.8%) — 좁은 에메랄드 빔 3개 다발 (각도 살짝 다름, dmg ×0.9)
- 🐲 사키하네 후야 (~15.8%) — 보라 길고 좁은 레이저, 강한 단발 (dmg ×3.0)
- 🦇 아카네 리제 (~15.8%) — 빨간 빔, 슬로우 2.0초, 처치 시 60% 확률 HP+3 흡혈
- 🎤 아라하시 타비 (~15.8%) — 주황 빔 + 호밍 음표 동시 발사
- 💤 졸린 윤소민 (15% 꽝) — 회색 짧고 약한 빔, 30% 확률로 "불 안 켜짐"
- 👑 칸자키 히나 (6% **잭팟**) — 5방향 거대 핑크/금 빔(360°), dmg ×1.1, 슬로우 3초, 처치 시 HP+1
- 확률 합: 79% 5명 + 15% 꽝 + 6% 잭팟 = 100%
...
**3초 안에 12킬 연속 콤보 시 5초간 폭주 모드 — 주방 미친 셰프 (밸런스 패치 후)**

- 콤보 윈도우: 마지막 킬로부터 3초 안에 추가 킬해야 누적
- 12킬 도달 시 발동: 5초간 모든 무기 발사 속도 ×2 (cd 절반)
...
- HUD: 대시 옆 콤보 카운트 (대기 중 `🍳 X/12`) 또는 잔여 시간 (`🍳 X.Xs`)
...
*마지막 업데이트: 풀 밸런스 패스 (잭팟 너프, riko 버프, 룰렛 확률 조정, 지팡이 cd, 페이얼 게이지 좌하단 이동, 비글/싸비 외형, 보스 1 강화)*
*총 작업 시간: 약 60+ 세션, index.html ~4030줄*
